...
if [ "$NOGRAPHIC" -eq 1 ]; then
    qemu-system-x86_64 -cdrom aaron_os.iso -drive file=hd.img,format=raw,if=ide,index=0,media=disk -boot order=cd -m 256M -machine pc -audiodev pa,id=speaker -machine pcspk-audiodev=speaker -audiodev pa,id=sb16 -device sb16,audiodev=sb16 -netdev user,id=n1 -device rtl8139,netdev=n1 -object filter-dump,id=f1,netdev=n1,file=aaronos.pcaps -nographic -device loader,addr=0x500,data=0xBB,data-len=1 -no-reboot
    echo "Exit with Ctrl+A then X"
else
    qemu-system-x86_64 -cdrom aaron_os.iso -drive file=hd.img,format=raw,if=ide,index=0,media=disk -boot order=d -m 256M -machine pc -audiodev pa,id=speaker -machine pcspk-audiodev=speaker -audiodev pa,id=sb16 -device sb16,audiodev=sb16 -netdev user,id=n1 -device rtl8139,netdev=n1 -object filter-dump,id=f1,netdev=n1,file=aaronos.pcaps
fi