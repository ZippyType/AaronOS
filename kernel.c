...
void run_command(char* cmd) {
    if (kstrcmp(cmd, "help") == 0) print_help();
        ...
        *((uint32_t*)vendor) = ebx; *((uint32_t*)(vendor + 4)) = edx; *((uint32_t*)(vendor + 8)) = ecx;
        ...
        if (sb16_is_present()) {
            ...
            if (kstrcmp(sub, "dma") == 0) {
                print("SB16: Playing via DMA...\n");
                sb16_play_dma(wave, 4096, 8000);
            } else if (kstrcmp(sub, "pio") == 0) {
                print("SB16: Playing via PIO...\n");
                sb16_play_pio(wave, 4096, 8000);
            } else if (kstrcmp(sub, "test") == 0) {
                print("SB16: Playing via DMA...\n");
                sb16_play_dma(wave, 4096, 8000);
            } else { print("Usage: sb16 [dma|pio|test]\n"); }
    ...
    else if (kstrncmp(cmd, "volume", 6) == 0) {
        if (sb16_is_present()) {
            if (cmd[6] == '\0') {
                char b1[8], b2[8];
                uint8_t mv = sb16_get_master_volume();
                uint8_t dv = sb16_get_dac_volume();
                print("Master: "); itoa(mv >> 4, b1, 10); print(b1);
                print(":"); itoa(mv & 0x0F, b2, 10); print(b2);
                print("  DAC: "); itoa(dv >> 4, b1, 10); print(b1);
                print(":"); itoa(dv & 0x0F, b2, 10); print(b2);
                print("\n");
            } else {
                char* arg = &cmd[7];
                if (kstrncmp(arg, "dac ", 4) == 0) {
                    int v = katoi(&arg[4]);
                    if (v >= 0 && v <= 15) { sb16_set_dac_volume(v, v); print("DAC volume set.\n"); }
                    else print("Usage: volume dac 0-15\n");
                } else {
                    int v = katoi(arg);
                    if (v >= 0 && v <= 15) { sb16_set_volume(v, v); print("Master volume set.\n"); }
                    else print("Usage: volume [dac] 0-15\n");
                }
            }
        } else { print("SB16 not available.\n"); }
